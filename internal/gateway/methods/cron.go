...
func (m *CronMethods) handleCreate(ctx context.Context, client *gateway.Client, req *protocol.RequestFrame) {
	...
	var params struct {
		...
		StaticMessage  *store.CronStaticMessageSpec `json:"staticMessage"` // set → fixed broadcast payload (no LLM)
	...
	isStaticMessage := params.StaticMessage != nil
	...
	} else if isStaticMessage {
		if err := store.ValidateCronStaticMessageSpec(params.StaticMessage); err != nil {
			client.SendResponse(protocol.NewErrorResponse(req.ID, protocol.ErrInvalidRequest, err.Error()))
			return
		}
		if params.DeliverChannel == "" {
			client.SendResponse(protocol.NewErrorResponse(req.ID, protocol.ErrInvalidRequest, i18n.T(locale, i18n.MsgRequired, "deliverChannel")))
			return
		}
	...
	}
	{
		...
		if isStaticMessage {
			patch.StaticMessage = params.StaticMessage
		}
...
func (m *CronMethods) handleUpdate(ctx context.Context, client *gateway.Client, req *protocol.RequestFrame) {
	...
	// A static message payload on update must be valid.
	if params.Patch.StaticMessage != nil {
		if err := store.ValidateCronStaticMessageSpec(params.Patch.StaticMessage); err != nil {
			client.SendResponse(protocol.NewErrorResponse(req.ID, protocol.ErrInvalidRequest, err.Error()))
			return
		}
	}
