...
// deterministic shell command ("command", no model cost), or sends a fixed
// broadcast message to multiple chat targets ("static_message", no model cost).
const (
	CronPayloadKindAgentTurn    = "agent_turn"
	CronPayloadKindCommand      = "command"
	CronPayloadKindStaticMessage = "static_message"
)
...
// CronStaticMessageSpec is a fixed text+image broadcast sent to one or more
// chat targets on the same channel, with no LLM involved (zero model tokens).
...
type CronPayload struct {
	...
	StaticMessage    *CronStaticMessageSpec  `json:"staticMessage,omitempty" db:"-"` // set when Kind=="static_message" (fixed broadcast)
...
// IsStaticMessage reports whether this payload sends a fixed broadcast message.
func (p CronPayload) IsStaticMessage() bool {
	return p.Kind == CronPayloadKindStaticMessage
}
...
// ValidateCronStaticMessageSpec checks structural validity of a static message payload.
func ValidateCronStaticMessageSpec(spec *CronStaticMessageSpec) error {
	if spec == nil {
		return fmt.Errorf("%w: static message spec required", ErrCronCommandInvalid)
	}
	if strings.TrimSpace(spec.Message) == "" && len(spec.Images) == 0 {
		return fmt.Errorf("%w: message or at least one image required", ErrCronCommandInvalid)
	}
	if len(spec.Targets) == 0 {
		return fmt.Errorf("%w: at least one target group/chat required", ErrCronCommandInvalid)
	}
	if len(spec.Images) > 9 {
		return fmt.Errorf("%w: max 9 images per message", ErrCronCommandInvalid)
	}
	return nil
}
...
type CronJobResult struct {
	...
	Summary      string `json:"summary,omitempty" db:"-"` // brief summary (used by static_message)
	InputTokens  int    `json:"inputTokens,omitempty" db:"-"`
	OutputTokens int    `json:"outputTokens,omitempty" db:"-"`
	DurationMS   int64  `json:"durationMs,omitempty" db:"-"`
}
...
type CronJobPatch struct {
	...
	StaticMessage  *CronStaticMessageSpec  `json:"staticMessage,omitempty" db:"-"` // set → switches the job to a fixed broadcast payload
