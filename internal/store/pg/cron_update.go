...
func (s *PGCronStore) UpdateJob(ctx context.Context, jobID string, patch store.CronJobPatch) (*store.CronJob, error) {
	...
	if patch.Message != "" || patch.Command != nil || patch.StaticMessage != nil {
		...
		if patch.Command != nil {
			...
			payload.StaticMessage = nil
		}
		if patch.StaticMessage != nil {
			payload.Kind = store.CronPayloadKindStaticMessage
			payload.StaticMessage = patch.StaticMessage
			payload.Command = nil
		}
